...
threshold =  [0, 1, 2, 100000]
values =  [0.6860554999999999, 1.496871, 4.239886]



def _read_img_model():
    model = ConvModel(1)
    model.load_state_dict(torch.load(_folder / 'termicheskaya.pth', map_location='cpu'))
    model = model.eval()
    return model

def _read_supported_details():
    with open(_folder / 'termicheskaya_supported_details.pkl', 'rb') as f:
        return pickle.load(f)

def _read_clf_cl():
    with open(_folder / 'termicheskaya_clf.pkl', 'rb') as f:
        return pickle.load(f)
