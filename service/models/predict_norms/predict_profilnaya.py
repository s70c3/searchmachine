...
from .predict_profilnaya_model import ConvModel
from . import common
...
values = [0.060624999999999984, 0.278, 0.5246445000000001, 0.7719630000000001, 1.3303999999999998, 3.05513]
...
def _read_supported_details():
    with open(_folder / 'profilnaya_supported_details.pkl', 'rb') as f:
        return pickle.load(f)

def _read_clf_cl():
    with open(_folder / 'profilnaya_clf.pkl', 'rb') as f:
        return pickle.load(f)
...
def predict_profilnaya(img_projections, mass, detail_name, thickness):
...
if config.run.models.norms:
    ...
    ## read models ##
    #################
    _folder = Path('./models/predict_norms/files/')
